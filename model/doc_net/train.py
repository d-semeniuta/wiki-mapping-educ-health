...
def generate_loaders(countries):
    ...
    for country in countries:
        data = Doc2VecAfricaDataset(countries=country)
        others = [c for c in countries if c is not country]
        others_data = Doc2VecAfricaDataset(countries=others)
    ...
    alldata = Doc2VecAfricaDataset(countries = countries)
    data_loaders['all'] = {}
...
def train_model(params, train_loader, val_loader, writer):
    ...
    for task in model_dict.keys():
        model = Doc2VecNet(task, params).to(device)
